...
- **Right sidebar — the navigator**, five tabs for the selected repo:
  *Files*, *Changes*, *Terminals*, *Claude*, *Info*. Collapsible (⌥⌘0).
...
| Reading while a reply lands | The transcript **follows the reply down only while you are at the bottom of it**. Scroll up to read something and it stops following — a paragraph landing every second no longer throws you back to the end mid-sentence — and scrolling back to the bottom starts it again. While you are up there a **↓ button** floats in the bottom corner of the transcript: it is the only sign that the end has moved on without you, and clicking it glides back down and takes the button with it. The one thing that always wins is **a prompt you just sent**: that re-parks the transcript wherever you had left it, because you sent it to see the answer. **Coming back to a conversation always lands on its newest message**, whatever position you left it at — every open chat stays in the view tree and is only made transparent while something else is on screen, so without this it would come back exactly where it was. How far the content's bottom is from the pane's is asked of the `NSScrollView` itself, and only a **scroll** is listened for: a reply growing raises nothing, so it can never unpin the view by itself. |
...
Sources/Workspace/
  ...
  Views/                      projects sidebar, navigator, viewer, diff, PR, info
    ...
    ClaudeWorkingLine.swift   the sparkle, rotating word and clock of a running turn
    ClaudeToolRow.swift       one tool call: dot, name, argument, diff or output
    ChatInputField.swift      the growing text box the chat and the comment boxes share
