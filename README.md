...
- **Left sidebar — Repositories.** One card per repo you added: host, branch,
  ...
  so does its square on the folded rail. Every card **reads itself again every
  five minutes** — branch, changed files and open pull requests — so a merge or
  a checkout made elsewhere shows up without pressing anything; ⌘R still sweeps
  the lot at once. Its header row carries the **home terminal button** (⇧⌘T) and
  nothing else. Collapsible (⌘0).
...
| Sidebar kept current | Every repository is **read again every five minutes** — the branch it is on with its ahead/behind, how many files have changed, how many pull requests are open — so a merge landed by somebody else, or a branch checked out in the terminal below, reaches the card without anything being pressed. **Only while the app is in front**: a pull request read costs a call to GitHub or Bitbucket per repository, and a workspace left open behind a browser for a week would spend thousands of them on a sidebar nobody is looking at. A sweep that fell due while the app was away goes out when you come back to it — one sweep, however long you were gone. **Refresh All** (⌘R) still reads everything at once, and pushes the next automatic sweep five minutes out. **Nothing shows that it is happening** — no spinner goes up for a read nobody asked for, and nothing already drawn is blanked; a number simply changes when the answer differs. The spinner is kept for the reads you ask for, where it says the button did something. |
...
| Diffs | `git diff` for working-tree changes (per file or **all changes at once**) and `gh/bkt pr diff` for PRs, rendered **side by side** (default) or unified, with **tree-sitter syntax colours**. A changed line is coloured in **two tiers** — a wash over the whole line, and a stronger block behind the **words that actually differ**, matched token by token, so an edited argument stands out from the rest of the line it sits in. **Reading the working tree is one place in the history, however many files it takes**: clicking down the Changes list replaces the diff in the pane rather than stacking an entry per file, so ⎋ (or the ✕) hands the window straight back to whatever sent you into the diff instead of stepping back through every file you read, and Back does the same. The layout you are reading in — unified or split — carries from one file to the next. A commit and a pull request are their own pages and keep their place in the history. |
...
| New file or folder | **+** in the Files tab's bottom bar makes an empty **file** or **folder** — in the folder picked in the tree, in the folder of the picked file, or in the repository itself when nothing is picked (the tooltip names which). What it makes arrives **at the top of its folder** — not wherever `untitled` sorts, which is a row you would have to go looking for — and **with its name already being edited**, so the name is typed once rather than into a sheet and then again if it was wrong; ⏎ commits it and ⎋ leaves an `untitled` behind, which the same box renames. Naming it keeps it at the top; it falls back into alphabetical order as soon as the selection moves off it. A closed folder springs open to show it, the list scrolls to it, and pressing **+** twice gives `untitled` and `untitled 2` rather than an error. |
...
| Editor | [CodeEditSourceEditor](https://github.com/CodeEditApp/CodeEditSourceEditor) — the editor CodeEdit itself uses: **tree-sitter** highlighting, gutter with line numbers and a **folding ribbon**, caret-line highlight, auto-indent, bracket matching, multiple cursors, **find and replace** (⌘F), and soft wrap toggle. It lays out only what is on screen, which is why a long file scrolls without stalling. A file the editing stack cannot take on in full — over **1 MB**, or carrying a **line longer than 5,000 characters**, which a minified bundle like `mermaid.min.js` is both of — opens **read-only** instead: wrapped, numbered, and plain, with **no highlighting and no language server**, and the status bar says why. |
...
| Language servers | Started per language and project root, warmed up as soon as a repository is selected: **diagnostics** (error and warning counts in the status bar), **completions** (⌃Space, or a trigger character like `.`), **⌘-click go-to-definition** — which opens the other file at the line, or moves the caret when the definition is in this one — and the **document symbol list**. Edits reach the server **incrementally**, batched every 250 ms, so typing does not make it re-read the file. See the table below, and `Docs/LSP.md` for how it fits together. |
...
| Claude tab | The conversations this repo has **running** in this window, then every conversation Claude Code has ever had about it, newest first. A running one is a terminal tab: click its row and that shell comes back on screen, and the **✕ under the pointer** ends that one alone. **A running conversation is called what it is about, not "Claude Code"** — the name is the one **Claude Code chose for the conversation itself**, read back from the same transcript the history below is titled from, so one conversation reads the same whether it is running or over. It appears once the first prompt has landed, with that prompt standing in for the couple of seconds before the CLI has settled on a name of its own (and for a transcript old enough to carry none); until then the shell's own tab name stands in, and the terminal's header row shows the same name. The transcript is read **a line at a time** rather than by its first few pages: a pasted screenshot is a megabyte of base64 on the same line as the prompt beside it, and a conversation that opened with an image used to end up called *Untitled conversation*. A row that is **mid-turn says *working…*** rather than *running*, which is the same thing the repository's card is badged with — with several conversations open it is the only way to tell which one is still going. **A conversation is listed here and nowhere else** — it runs in a terminal tab, but the Terminals tab leaves it out, so the shells you opened yourself are not buried among chats and there is one way back to each. The **PAST** heading is a switch — click it and the whole history folds away, with the number of hidden conversations beside it — and the choice is **remembered between launches**. The history is read from `~/.claude/projects/…`, so **a session you started in a shell of your own is in the list too**, titled the same way and stamped with when it last moved. **Click one and it resumes in a terminal of its own** (`claude --resume`), leaving whatever else is running alone; clicking it again comes back to that same tab rather than starting a second `claude` on the same transcript. Nothing is launched by looking: the list costs no `claude` process. **New Conversation** sits at the top, and the **✕** on a past row (or *Move to Trash*) throws it away — transcript and the folder of tool output beside it, to the Trash rather than unlinked, so it does not stop to ask. |
...
| Terminals end with the app | Quitting closes them: a launch starts with **no shells and no conversations**, whatever was open last time. Tabs used to be saved and listed again, but the process behind one dies with the app, so what came back was an empty tab wearing a dead shell's name — and a Claude Code tab came back as a shell that had never run. Conversations that are still on disk are read from their own transcripts instead, in the Claude tab's **Past** list. |
...
| Settings (⌘,) → Appearance | The **theme and the font code is shown in**. Four themes ship with the app — **Adonis Eclipse**, **GitHub Dark**, **Atom One Dark** and **Dark+** — each a full palette for the editor and the diff, including the background, gutter, current line and indent guides, with a row of swatches under the picker so two dark themes can be told apart at a glance. They travel with the app, so a file looks the same on every Mac you run it on. `Scripts/import-vscode-theme.swift "Some Theme"` ports any VS Code theme into a new one — it reads the extension's own theme file and translates its TextMate scopes into the tree-sitter captures our highlighter produces. **The theme paints the terminal too** — see the row below. Then the fonts: **one face for the editor, the diff and the terminal**, with a size of its own for the diff (denser on purpose) and **line spacing** for the editor; a shell is drawn in that face at the editor's size, so there is no separate terminal section to keep in step. Only **monospaced faces** are listed — Fira Code, JetBrains Mono, Monaspace and the rest are found even though those families leave the `monoSpace` trait unset — and each is drawn in itself in the menu. Every change applies at once: the editor re-lays out, and libghostty is handed a new configuration, so **shells already running** change with it. Kept in `UserDefaults`; **Restore Defaults** puts back SF Mono at 12.5 / 12 pt and ×1.1 spacing. |
| The terminal wears the theme | A shell takes **the theme picked in Settings**: its background is the editor's background, its text the editor's text, its cursor the editor's caret, its selection the editor's selection band — and **all sixteen ANSI colours** come from the theme as well, so `ls`, `git diff` and a prompt are coloured in the same palette the code beside them is. A syntax theme names no ANSI slots, so each one takes **the theme colour nearest it on the hue wheel**: red is whatever red that theme writes errors and keywords in, green whatever it uses for strings. Nothing has to know which capture a theme decided to make green, and a theme with only three hues in it keeps plain defaults for the rest rather than filling six slots with the same colour. The bright half is those colours with the lamp turned up. **Changing the theme repaints the shells already running** — libghostty is handed a new configuration, not restarted. Your own `~/.config/ghostty/config` still decides everything the app says nothing about (keybindings, shell integration); it no longer decides the colours or the font. |
