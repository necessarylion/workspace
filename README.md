...
| New file or folder | **+** in the Files tab's bottom bar makes an empty **file** or **folder** — in the folder picked in the tree, in the folder of the picked file, or in the repository itself when nothing is picked (the tooltip names which). What it makes arrives **with its name already being edited**, on the row, so the name is typed once rather than into a sheet and then again if it was wrong; ⏎ commits it and ⎋ leaves an `untitled` behind, which the same box renames. A closed folder springs open to show it, and pressing **+** twice gives `untitled` and `untitled 2` rather than an error. |
